...
class SectorAnalysisSection {
    ...
    async updateSectorMetrics() {
        try {
            const metrics = await this.getSectorMetrics(this.currentSector);
            
            document.getElementById('sectorMarketCap').textContent = this.formatMarketCap(metrics.totalMarketCap);
            document.getElementById('sectorVolume').textContent = this.formatVolume(metrics.volume24h);
            document.getElementById('sectorChange').textContent = this.formatPercentage(metrics.dominanceIndex);
            document.getElementById('sectorProjects').textContent = metrics.volatilityIndex.toString();

            // Update class for color coding
            const changeElement = document.getElementById('sectorChange');
            changeElement.classList.remove('positive', 'negative');
            changeElement.classList.add(metrics.dominanceIndex >= 0 ? 'positive' : 'negative');
        } catch (error) {
    ...
    async updateSectorInsights() {
        try {
            const insights = await this.generateSectorInsights(this.currentSector);
            
            document.getElementById('sectorTrends').textContent = insights.trend;
            document.getElementById('marketStructure').textContent = insights.summary;
            document.getElementById('sectorOpportunities').textContent = insights.opportunities.join(', ');
        } catch (error) {
    ...
    async generateSectorInsights(sector) {
        // Implement your preferred data source here
        return {
            trend: 'neutral',
